...
const SearchMovies: React.FC = () => {
  ...
  const searchMoviesData = async (query: string) => {
    try {
      const response = await fetch(`https://api.themoviedb.org/3/search/movie?api_key=3c6589d335c0f9d7f3d59fba6a8ea553&query=${query}&language=en-US&sort_by=popularity.desc&page=1`);
      ...
      if (data.results) {
        ...
        // Show only the 12 most popular results
        const top12Movies = filteredMovies.slice(0, 12);
  
        setMovies(top12Movies);
      }
    } catch (error) {
